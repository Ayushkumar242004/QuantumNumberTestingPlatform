...
class BirthdaySpacingsTest:
    ...
    def BirthdaySpacingsTest(data, t=512, verbose=False):
        ...
        try:
            ...
            # Parallel sorting of each chunk with timeout
            with ThreadPoolExecutor() as executor:
                futures = {
                    executor.submit(np.sort, mod_data[i * chunk_size:(i + 1) * chunk_size]): i
                    for i in range(num_chunks)
                }
                for future in as_completed(futures, timeout=BirthdaySpacingsTest.MAX_TIME_LIMIT):
                    try:
                        sorted_chunks.append(future.result(timeout=BirthdaySpacingsTest.MAX_TIME_LIMIT))
                    except TimeoutError:
                        print(f"Timeout occurred for chunk {futures[future]}")
                        return -3, False  # Indicate timeout failure
