    ...
    def autocorrelation_test(binary_data: str, max_lag: int = 20, verbose=False):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in as_completed(futures):
                lag = futures[future]
                try:
                    autocovariance = future.result(timeout=2)  # Set max time limit
                    autocorrelations.append(autocovariance / variance)  # Normalize by variance
                except TimeoutError:
                    print(f"Warning: Autocovariance computation for lag {lag} exceeded time limit.")
                    autocorrelations.append(0)  # Default to 0 if timeout occurs
