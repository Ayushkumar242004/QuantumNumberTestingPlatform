...
class CrapsTest:
    ...
    def process_chunk(data):
        """Vectorized computation of Craps test outcomes."""
        wins, losses = 0, 0
        i = 0
        n = len(data)
        ...
        while i + 1 < n:
            ...
            if outcome in [2, 3, 12]:  # Craps (loss)
                losses += 1
            elif outcome in [7, 11]:  # Natural win
                wins += 1
            else:  # Point phase
                point = outcome
                while True:
                    i += 2
                    if i + 1 >= n:
                        return wins, losses  # No more dice rolls left
    ...
    def CrapsTest(data, verbose=False):
        print("craps")
        """Performs an optimized Craps test with efficient batch processing."""
        ...
        # Step 2: Process all data in a single pass
        wins, losses = CrapsTest.process_chunk(data)

        # Step 3: Compute statistical test
        total_games = wins + losses
        if total_games == 0:
            return -2, False  # No games played

        expected_wins = total_games * (244 / 495)  
        variance = total_games * (244 / 495) * (1 - (244 / 495))
