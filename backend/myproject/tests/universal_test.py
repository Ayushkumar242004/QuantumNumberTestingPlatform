...
class Universal:
    MAX_EXECUTION_TIME = 2  # Maximum allowed time for processing each block (in seconds)
    ...
    def process_block(block_data, i, vobs, init_bits):
        try:
            ...
            return 0  # Return 0 if an error occurs during processing
    ...
    def statistical_test(binary_data: str, verbose=False):
        ...
        if 5 < pattern_size < 16:
            ...
            with ThreadPoolExecutor() as executor:
                ...
                for future in as_completed(futures):
                    try:
                        cumsum += future.result(timeout=Universal.MAX_EXECUTION_TIME)
                    except TimeoutError:
                        print(f"Block processing timed out at index {futures[future]}")
