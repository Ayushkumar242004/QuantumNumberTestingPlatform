...
double BlockFrequency(int M, int n, int epsilon[])
{
    int i, j, N, blockSum;
...
int main(int argc, char *argv[]) {
    ...
    int pass_count = 0;
    for (int i = 0; i < num_streams; i++) {
        double p = BlockFrequency(M, n, &epsilon[i * n]);
        p_values[i] = p;
    ...
    // Output in same format as frequency_test: "<chi_p> <final_pass>"
    printf("%.6f %d\n", chi_p, final_pass);
