...
class Ranks32x32MatricesTest:
    ...
    def Ranks32x32MatricesTest(data, verbose=False):
        print("32")
        """
        Efficiently computes the Ranks 32x32 Matrices test using vectorized NumPy operations.
        """
        ...
        # Step 2: Ensure we have enough data for at least one 32x32 matrix
        if len(data) < 32 * 32:
            return -2, False  
        ...
        # Step 4: Reshape into 32x32 matrices
        n = len(data) // (32 * 32)
        if n == 0:
            return -2, False  

        matrices = data[:n * 32 * 32].reshape(n, 32, 32)

        # Step 5: Compute ranks using vectorized NumPy operations
        ranks = np.linalg.matrix_rank(matrices, hermitian=False)

        # Step 6: Count occurrences of each rank
        rank_32 = np.sum(ranks == 32)
        rank_31 = np.sum(ranks == 31)
        rank_less_31 = n - (rank_32 + rank_31)

        counts = np.array([rank_32, rank_31, rank_less_31])

        # Step 7: Expected counts based on known rank proportions
        expected_proportions = np.array([0.3230, 0.5744, 0.1026])
        expected_counts = n * expected_proportions

        # Step 8: Chi-square test for goodness of fit
        chi_square = np.sum((counts - expected_counts) ** 2 / expected_counts)
        p_value = 1 - chi2.cdf(chi_square, 2)

        if verbose:
            print(f"Ranks 32x32 Matrices Test - p-value: {p_value}")
