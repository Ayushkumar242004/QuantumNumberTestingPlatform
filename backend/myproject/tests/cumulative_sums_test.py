...
class CumulativeSums:
    ...
    def cumulative_sums_test(binary_data: str, mode=0, verbose=False):
        ...
        try:
            ...
            num_chunks = min(8, ceil(length_of_binary_data / 1000))
            if num_chunks == 0:
                if verbose:
                    print("Error: num_chunks is 0, binary_data is too short.")
                return -4, False
            ...
            with ThreadPoolExecutor() as executor:
                ...
                for future, i in futures:
                    try:
                        result = future.result(timeout=CumulativeSums.MAX_TIME_LIMIT)
                    except TimeoutError:
                        print(f"Warning: Chunk {i} processing timed out.")
                        result = np.zeros(chunk_size)
                    except Exception as e:
                        print(f"Error in processing chunk {i}: {e}")
                        return -4, False
            ...
            terms_one = [
                norm.cdf((4 * k + 1) * abs_max / np.sqrt(length_of_binary_data)) -
                norm.cdf((4 * k - 1) * abs_max / np.sqrt(length_of_binary_data))
                for k in range(start, end + 1)
            ]
            ...
            terms_two = [
                norm.cdf((4 * k + 3) * abs_max / np.sqrt(length_of_binary_data)) -
                norm.cdf((4 * k + 1) * abs_max / np.sqrt(length_of_binary_data))
                for k in range(start, end + 1)
            ]
