...
// FFT routines (from Ogg Vorbis or Numerical Recipes – you must link or implement)
void __ogg_fdrffti(int n, double *wsave, int *ifac);
void __ogg_fdrfftf(int n, double *X, double *wsave, int *ifac);

// Simple erfc-based helper if unavailable
double cephes_erfc(double x) {
    return erfc(x);
}

void DiscreteFourierTransform(int n, int *epsilon) {
    double p_value, upperBound, percentile, N_l, N_o, d;
    double *X = NULL, *m = NULL, *wsave = NULL;
    int i, count, ifac[15];
    ...
    percentile = (double)count / (n / 2) * 100.0;
    N_l = (double)count;
    N_o = 0.95 * n / 2.0;
...
int main(int argc, char *argv[]) {
    ...
    DiscreteFourierTransform(n, epsilon);
    free(epsilon);
    return 0;
}
