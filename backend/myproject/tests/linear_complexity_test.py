...
class ComplexityTest:
    ...
    MAX_EXECUTION_TIME = 15  # Maximum time limit per block in seconds
    ...
    def linear_complexity_test(binary_data: str, verbose=False, block_size=5):
        ...
        try:
            ...
            if number_of_block > 1:
                ...
                with ThreadPoolExecutor() as executor:
                    futures = {executor.submit(ComplexityTest.berlekamp_massey_algorithm, block): block for block in blocks}
                    for future in as_completed(futures, timeout=ComplexityTest.MAX_EXECUTION_TIME):
                        try:
                            result = future.result(timeout=ComplexityTest.MAX_EXECUTION_TIME)
                            if isinstance(result, tuple) and result[0] != -1:
                                complexities.append(result[0])
                        except TimeoutError:
                            return (-1, False, 'Error: Test timed out')
