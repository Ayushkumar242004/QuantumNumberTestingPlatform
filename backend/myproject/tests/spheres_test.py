...
class Spheres3DTest:
    ...
    def Spheres3DTest(data, verbose=False):
        print("spheres")
        """
        Efficiently computes the 3D Spheres test using vectorized NumPy operations.
        """
        ...
        # Step 2: Ensure we have at least one full 3D point
        if len(data) < 3:
            return -2, False  
        ...
        # Step 4: Reshape into 3D points (x, y, z)
        n = len(data) // 3  
        if n == 0:
            return -2, False  
        ...
        # Step 5: Compute Euclidean distances using vectorized norm
        distances = np.linalg.norm(points, axis=1)

        # Step 6: Count points inside the unit sphere using Boolean masking
        inside_sphere = np.sum(distances <= 1)

        # Step 7: Compute expected value and variance
        expected = n * (4 / 3) * np.pi  
        variance = n * ((4 / 3) * np.pi) * (1 - (4 / 3) * np.pi)

        if variance <= 0:
            return -1, False  

        # Step 8: Calculate Z-statistic and p-value
        z_statistic = (inside_sphere - expected) / np.sqrt(variance)
        p_value = 2 * (1 - norm.cdf(abs(z_statistic)))

        if verbose:
            print(f"3D Spheres Test - p-value: {p_value}")
