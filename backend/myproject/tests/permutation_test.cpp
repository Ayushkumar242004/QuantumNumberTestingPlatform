...
public:
    struct PermutationTestResult {
        int iteration;
        long double excursion;
        long double numDirectionalRuns;
        long double lenDirectionalRuns;
        long double numIncreasesDecreases;
        long double numRunsMedian;
        long double lenRunsMedian;
        long double avgCollision;
        long double maxCollision;
        long double periodicity1;
        long double periodicity2;
        long double periodicity8;
        long double periodicity16;
        long double periodicity32;
        long double covariance1;
        long double covariance2;
        long double covariance8;
        long double covariance16;
        long double covariance32;
...
unsigned int periodicity(const uint8_t data[], const unsigned int p, const unsigned int n){
	unsigned int T = 0;
	 ...
	 if (n < p) {
        std::cerr << "Error: n = " << n << ", p = " << p << std::endl;
        assert(n >= p);
    }
...
void populateTestCase(IidTestCase &tc, int C[][3]){
    IidTestCase::PermutationTestResult tr0, tr1, tr2;
    
    tr0.iteration = 0; tr1.iteration = 1; tr2.iteration = 2;
    tr0.excursion = C[0][0]; tr1.excursion = C[0][1]; tr2.excursion = C[0][2];
    tr0.numDirectionalRuns = C[1][0]; tr1.numDirectionalRuns = C[1][1]; tr2.numDirectionalRuns = C[1][2];
    tr0.lenDirectionalRuns = C[2][0]; tr1.lenDirectionalRuns = C[2][1]; tr2.lenDirectionalRuns = C[2][2];
    tr0.numIncreasesDecreases = C[3][0]; tr1.numIncreasesDecreases = C[3][1]; tr2.numIncreasesDecreases = C[3][2];
    tr0.numRunsMedian = C[4][0]; tr1.numRunsMedian = C[4][1]; tr2.numRunsMedian = C[4][2];
    tr0.lenRunsMedian = C[5][0]; tr1.lenRunsMedian = C[5][1]; tr2.lenRunsMedian = C[5][2];
    tr0.avgCollision = C[6][0]; tr1.avgCollision = C[6][1]; tr2.avgCollision = C[6][2];
    tr0.maxCollision = C[7][0]; tr1.maxCollision = C[7][1]; tr2.maxCollision = C[7][2];
    tr0.periodicity1 = C[8][0]; tr1.periodicity1 = C[8][1]; tr2.periodicity1 = C[8][2];
    tr0.periodicity2 = C[9][0]; tr1.periodicity2 = C[9][1]; tr2.periodicity2 = C[9][2];
    tr0.periodicity8 = C[10][0]; tr1.periodicity8 = C[10][1]; tr2.periodicity8 = C[10][2];
    tr0.periodicity16 = C[11][0]; tr1.periodicity16 = C[11][1]; tr2.periodicity16 = C[11][2];
    tr0.periodicity32 = C[12][0]; tr1.periodicity32 = C[12][1]; tr2.periodicity32 = C[12][2];
    tr0.covariance1 = C[13][0]; tr1.covariance1 = C[13][1]; tr2.covariance1 = C[13][2];
    tr0.covariance2 = C[14][0]; tr1.covariance2 = C[14][1]; tr2.covariance2 = C[14][2];
    tr0.covariance8 = C[15][0]; tr1.covariance8 = C[15][1]; tr2.covariance8 = C[15][2];
    tr0.covariance16 = C[16][0]; tr1.covariance16 = C[16][1]; tr2.covariance16 = C[16][2];
    tr0.covariance32 = C[17][0]; tr1.covariance32 = C[17][1]; tr2.covariance32 = C[17][2];
    tr0.compression = C[18][0]; tr1.compression = C[18][1]; tr2.compression = C[18][2];
...
bool permutation_tests(const data_t *dp, const double rawmean, const double median, const int verbose, IidTestCase &tc){
    ...
    if(verbose >= 2) {
        // cout << "Initial test results:" << endl;
        for(unsigned int i = 0; i < num_tests; i++){
            // cout << setw(23) << test_names[i] << ": " << t[i] << endl;
        }
        cout << endl << "Beginning permutation tests..." << endl;
    }

    #pragma omp parallel
    {
        uint8_t *data = new uint8_t[dp->len];
        uint8_t *rawdata = new uint8_t[dp->len];
        ...
        for(int i = 0; i < PERMS; ++i) {
            if(passed_count < 19) {
                // Copy original data
                memcpy(data, dp->symbols, dp->len);
                memcpy(rawdata, dp->rawsymbols, dp->len);
...
int main(int argc, char* argv[]) {
    ...
    data.blen = static_cast<int>(len);
    data.maxsymbol = 1;
    data.symbols = new uint8_t[len];
    data.rawsymbols = new uint8_t[len];
    data.bsymbols = new uint8_t[len];

    // Convert input string to binary data
    for (size_t i = 0; i < len; ++i) {
        uint8_t bit = (binary_input[i] == '1') ? 1 : 0;
        data.symbols[i] = bit;
        data.rawsymbols[i] = bit;
        data.bsymbols[i] = bit;
    }

    // Calculate statistics
    double rawmean = 0.0, median = 0.0;
