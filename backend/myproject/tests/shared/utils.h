...
bool read_file_subset(const char *file_path, data_t *dp, unsigned long subsetIndex, unsigned long subsetSize, TestRunBase *testRun) {
	...
	std::vector<int> symbol_map_down_table(max_symbols);

	// create symbols (samples) and check if they need to be mapped down
	dp->alph_size = 0;
	std::fill(symbol_map_down_table.begin(), symbol_map_down_table.end(), 0);
	mask = max_symbols-1;
	for(i = 0; i < dp->len; i++){ 
		dp->symbols[i] &= mask;
...
uint64_t randomRange64(uint64_t s, uint64_t *xoshiro256starstarState){
	...
	uint128_t m;
#else
	// Fallback: use two 64-bit multiplications to simulate 128-bit math (less precise)
	...
	if(UINT64_MAX == s) {
...
#else
		// Fallback: simulate 128-bit multiplication (not as precise as real uint128_t)
		...
		uint64_t d = xl * sl;
		m_high = a + (b >> 32) + (c >> 32);
		m_low = d + ((b & 0xFFFFFFFF) << 32) + ((c & 0xFFFFFFFF) << 32);
		l = m_low;
#endif
...
#else
		return m_high; // Fallback: return high 64 bits as approximation
#endif
	}
}
