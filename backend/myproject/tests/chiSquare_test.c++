...
protected:
	// If you have the JsonCpp library, make sure to include its header:
	// #include <json/json.h>
	// Otherwise, replace this function with a stub or remove it.

	// Stub implementation to avoid compile error if Json::Value is not available
	void GetBaseJson() {
		// Implementation removed or replaced due to missing Json::Value type.
		// You can implement this function if you add the JsonCpp library and include <json/json.h>
	}
};
...
bool read_file_subset(const char *file_path, data_t *dp, unsigned long subsetIndex, unsigned long subsetSize, TestRunBase *testRun) {
	...
	std::vector<int> symbol_map_down_table(max_symbols, 0);

	// create symbols (samples) and check if they need to be mapped down
	dp->alph_size = 0;
	// memset(symbol_map_down_table, 0, max_symbols*sizeof(int)); // Not needed with vector initialization
	mask = max_symbols-1;
	for(i = 0; i < dp->len; i++){ 
		dp->symbols[i] &= mask;
...
uint64_t randomRange64(uint64_t s, uint64_t *xoshiro256starstarState){
	...
	if(UINT64_MAX == s) {
...
#else
		// Portable 128-bit multiplication using 64-bit math
		uint64_t xh = x >> 32, xl = x & 0xFFFFFFFF;
		uint64_t sh = s >> 32, sl = s & 0xFFFFFFFF;
		uint64_t a = xh * sh;
		uint64_t b = xh * sl;
		uint64_t c = xl * sh;
		uint64_t d = xl * sl;
		...
		uint64_t mid = b + c;
		uint64_t mid_carry = ((mid < b) ? 1ULL : 0ULL) << 32;
		m_high = a + (mid >> 32) + mid_carry;
		m_low = (mid << 32) + d;
		if(m_low < d) m_high++;
		l = m_low;
#endif
		...
		if(l<s) {
			...
			while(l < t) {
				...
				l = (uint64_t)m; //This is m mod 2^64
#else
				xh = x >> 32; xl = x & 0xFFFFFFFF;
				sh = s >> 32; sl = s & 0xFFFFFFFF;
				a = xh * sh;
				b = xh * sl;
				c = xl * sh;
				d = xl * sl;
				mid = b + c;
				mid_carry = ((mid < b) ? 1ULL : 0ULL) << 32;
				m_high = a + (mid >> 32) + mid_carry;
				m_low = (mid << 32) + d;
				if(m_low < d) m_high++;
				l = m_low;
