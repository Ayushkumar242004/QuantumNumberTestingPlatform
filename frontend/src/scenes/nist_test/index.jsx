  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      try {
        ...
        // Post the request to start processing
        const response = await axios.post("http://localhost:8000/generate_final_ans/", {
      ...
      try {
        progressInterval = setInterval(async () => {
          try {
            ...
            const completed = progressRes.data.progress || 0;
            const percent = Math.round((completed / 17) * 100);
            setLoadingProgress6(prev => (percent > prev ? percent : prev)); // Prevent regressions
          } catch (err) {
            alert(`Error: ${err}`);
          }
        }, 1000);
      ...
      try {
        progressInterval = setInterval(async () => {
          try {
            ...
            const completed = progressRes.data.progress || 0;
            const percent = Math.round((completed / 17) * 100);
            setLoadingProgress7(prev => (percent > prev ? percent : prev)); // Prevent regressions
          } catch (err) {
            alert(`Error: ${err}`);
          }
        }, 1000);
      ...
      try {
        progressInterval = setInterval(async () => {
          try {
            ...
            const completed = progressRes.data.progress || 0;
            const percent = Math.round((completed / 17) * 100);
            setLoadingProgress8(prev => (percent > prev ? percent : prev)); // Prevent regressions
          } catch (err) {
            alert(`Error: ${err}`);
          }
        }, 1000);
      ...
      try {
        progressInterval = setInterval(async () => {
          try {
            ...
            const completed = progressRes.data.progress || 0;
            const percent = Math.round((completed / 17) * 100);
            setLoadingProgress9(prev => (percent > prev ? percent : prev)); // Prevent regressions
          } catch (err) {
            alert(`Error: ${err}`);
          }
        }, 1000);
      ...
      try {
        progressInterval = setInterval(async () => {
          try {
            ...
            const completed = progressRes.data.progress || 0;
            const percent = Math.round((completed / 17) * 100);
            setLoadingProgress10(prev => (percent > prev ? percent : prev)); // Prevent regressions
          } catch (err) {
            alert(`Error: ${err}`);
          }
        }, 1000);
